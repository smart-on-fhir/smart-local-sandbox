...
const CFG = {
    "stu2": {
        server: "http://localhost:4002/baseDstu2",
        pickerConfigFile: "/usr/app/ext/smart-launcher/static/patient-browser/config/dstu2-local.json5"
    },
    "stu3": {
        server: "http://localhost:4003/baseDstu3",
        pickerConfigFile: "/usr/app/ext/smart-launcher/static/patient-browser/config/stu3-local.json5"
    }
};
...
function getAllPages(options, cb, result = []) {
    request(options, (error, response, bundle) => {
        ...
        process.stdout.write(
            `${Math.floor(result.length/bundle.total * 100)}`.hidden
        );
...
getAllPages({
    ...
    const filePath = CFG[app.stu].pickerConfigFile;
